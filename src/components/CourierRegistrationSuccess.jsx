...
export default function CourierRegistrationSuccess() {
  ...
  // courier passed from navigate state
  const courier = location.state || {};

  // courier id from params
  const { id } = useParams();

  // use param id first, fallback to state id
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#e2fffe]">
        <p className="text-[#001736] font-bold text-lg">Loading courier...</p>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#e2fffe]">
        <p className="text-red-500 font-bold">
          Failed to load courier. Please retry.
        </p>
      </div>
    );
  }
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-6 w-full mb-12">
          ...
          <div className="md:col-span-8 bg-[#ffffff] rounded-xl p-8 shadow-sm flex flex-col md:flex-row gap-8 items-center md:items-start">
            <div className="relative">
              <img
                src={
                  courierData?.profileImage || "https://via.placeholder.com/150"
                }
                alt={courierData?.fullName || "Courier"}
